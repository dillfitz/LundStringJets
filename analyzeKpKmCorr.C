...
void analyzeKpKmCorr()
{
	...
	// z histos //
  	...
  	// histograms differential in jet pT //
  	const int nPtBins = 3;
  	...
  	// kpkm correlations //
  	TH1D *kpairmass = new TH1D("kpairmass",";m (GeV/c^{2});" , 430, 0.8, 3.5);
  	TH1D *kpairangle = new TH1D("kpairangle",";#alpha;", 110, -0.2, 0.9);
	TH1D *kpairenergy = new TH1D("kpairenergy",";E;", 400, 0., 100);
	TH1D *kpairmom = new TH1D("kpairmom",";p;", 100, 0., 100);
	TH1D *kpairpt = new TH1D("kpairpt",";pt;", 300, 0., 30);
	//TH1D *kpkmz = new TH1D("kpairz",";z;", 100, 0,1);
    ...
    if (kpkmJet)
    {
    	for (int j=conEntry1; j<(conEntry1 + nConstituents); ++j)
    	{
			...
			for (int k=conEntry2; k<(conEntry2 + nConstituents); ++k)
        	{
				...
				TVector3 kpair_vect = kaon1_vect + kaon2_vect;
				...
				kpairenergy->Fill(kpair.E());
				kpairmom->Fill(kpair_vect.Mag());
				kpairpt->Fill(kpair.Pt());
				//kpkmz->Fill(kpair.Z());
				kpkmz->Fill(z1,z2);
				kpkmpt->Fill(pt1,pt2);
				kpkmpz->Fill(pz1,pz2);
